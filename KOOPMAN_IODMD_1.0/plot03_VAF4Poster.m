...
datDir = 'datOutputDir';
matFile0_WT = fullfile(datDir,'simAll1_plotStruct_long70.mat'); % Turbine data
matFile1_WT = fullfile(datDir,'simAll1_plotStruct_resampledOriginal_Kpsi24.mat'); %' Turbine data with lifted rotor speeds
...
tmp = load(matFile0_WT,'plotStruct');
plotStruct_WT0 = tmp.plotStruct;    
cl_WT = [1,0,0];
cl1_WT = [1,0,1];
visualizeVAFsub(1,plotStruct_WT0,matFile1_WT,scalingfactors,meanvalues,cl_WT,cl1_WT);
xticklabels([]); % Removes X-axis labels to save space
...
cl = [1,0,0];
cl1 = [1,0,1];
visualizeVAFsub(2,plotStruct_WT0,matFile1_WT,scalingfactors,meanvalues,cl_WT,cl1_WT);
xticklabels([]); % Removes X-axis labels to save space
...
legend(leg,'Location','eastoutside','FontSize',fs-1,'Box','off','NumColumns',1);
axis tight; grid on;
...
pos0 = get(0,'defaultFigurePosition');
set(gcf,'Position',[pos0(1:2),pos0(3)*1.7,pos0(4)*1.1])

strFig = 'aPowerEstSofwaInOutPoster';
...
function vec2 = visualizeVAFsub(idx0,plotStruct,matFile1,scalingfactors,meanvalues,cl1a, cl1b) 
...
plot(t,simPwr/10^6,'color',cl0(1,:),'LineWidth',1.5) ; hold on;
...
for sidx = vec2 % length(plotStruct)
    ...
    plot(t,simPwr/10^6,'color', cl1a,'linestyle',ls{2},'LineWidth',lw+0.2); % idx/2*[0,0,1]) ; hold on;
...
for sidx = 3 %vec2 % length(plotStruct)
    idx = vec(sidx);
    lw = lwVec(sidx);
    simPwr =  plotStruct{idx}.ysim_val(1:end-1,idx0) * scalingfactors(idx0) + meanvalues(idx0);
    plot(t,simPwr/10^6,'color', cl1b,'linestyle',ls{3},'LineWidth',lw); % idx/2*[0,0,1]) ; hold on;
