...
fprintf('Best performance T2, T1: %2.3f, %2.3f, model states %d\n',FITje_val(1,modeltouse),maxval,modeltouse);

purpose = ''; x = '';
OMEGA = ''; DAMPING = ''; 
%sys_red,si,Inputs, Outputs,FITje,OMEGA,DAMPING,purpose,x,states,U,Deterministic,method
evaluatemodel(sys_red,modeltouse,Inputs_val,Outputs_val,FITje_val,OMEGA,DAMPING,purpose,x,statesvalid,U,Deterministic_val,method)

 %[FITje,OMEGA,DAMPING,fig1,x]=evaluatemodel(sys_red,si,Inputs, Outputs,FITje,OMEGA,DAMPING,purpose,x,states,U,Deterministic,method)

%(sys_red,modeltouse,Inputs_val,Outputs_val,FITje_val,purpose,x,statesvalidDMD,U,Deterministic_val,method)
...
elseif pitchmode==1
    ...
    comparereconstruction_pitch(statesvalid(1:size(QQ_u,1)/2,:), statesrebuildvalid(1:size(QQ_u,1)/2,:),D,dirdmd,x,y,z,Decimate,dirName_val,'reconsversusSOWFA_valid',515,Inputs_val) %comparison at dosntream rotor plane
...
subpart = 3; [f] = MPC_progress(part,subpart,f,{},{});
evauatemodelerror(states, statesrebuild,D,dirdmd,filename,dirName,x,y,z,Decimate,pitchmode)

%evaluate time varying error in simualtions according to normalised
%root mean squared error at each grid lcoation
[nrmse, nrmsevalid]=...
    evaluatetimevaryingerror(states(1:size(QQ_u,1)/2,:),statesrebuild(1:size(QQ_u,1)/2,:),statesvalid(1:size(QQ_u,1)/2,:), statesrebuildvalid(1:size(QQ_u,1)/2,:),dirdmd,'statestimeerror');
