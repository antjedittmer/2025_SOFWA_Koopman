...
Xsel_WT = Xsel([1,end]); Xsel_noWT = Xsel([2,end-1]);
Xm_shs_2Dsel_WT = Xm_shs_2D(Ysel,Xsel_WT);
Xm_shs_2Dsel_noWT = Xm_shs_2D(Ysel,Xsel_noWT);
Ym_shs_2Dsel_WT = Ym_shs_2D(Ysel,Xsel_WT);
Ym_shs_2Dsel_noWT = Ym_shs_2D(Ysel,Xsel_noWT);
U_usel_WT = UmeanAbs_sh_u2D(Ysel,Xsel_WT);
U_vsel_WT = UmeanAbs_sh_v2D(Ysel,Xsel_WT);
U_usel_noWT = UmeanAbs_sh_u2D(Ysel,Xsel_noWT);
U_vsel_noWT = UmeanAbs_sh_v2D(Ysel,Xsel_noWT);
...
hSparse = quiver(Xm_shs_2Dsel_WT, Ym_shs_2Dsel_WT, U_usel_WT, U_vsel_WT, ...
    'r', 'AutoScaleFactor', 0.2, 'LineWidth', 1.2);
hSparse2 = quiver(Xm_shs_2Dsel_noWT, Ym_shs_2Dsel_noWT, U_usel_noWT, U_vsel_noWT, ...
    'g', 'AutoScaleFactor', 0.2, 'LineWidth', 1.2);
...
lgd = legend('Wind contour hub height (m/s)', ...
             ...
             'Wind data at turbines (m/s)',...
             'Additional wind data points (m/s)');
lgd.FontSize = fs;
colorbar(); drawnow;

strFig = 'WindData_Poster';
...
titles = {'WT1 Rotor (index 1)', ...
          ...
          sprintf('%2.1f m in front of WT2 (index %d)',890 - 12.5*(Xsel(4)- 1), Xsel(4))};
p2 = 1;
...
for idxT = 1:4
    ...
    quiver(Y_p(Ysel, Zsel), Z_p(Ysel, Zsel), ...
                     U_v(Ysel, Zsel), U_w(Ysel, Zsel), colStr, ...
...
strFig = 'WindData_Horizontal_Slices_Poster';
